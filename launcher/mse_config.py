"""Shared Magic Set Editor configuration loaded from launcher/.env."""
...
def write_env_file(env_path: Path, values: Mapping[str, str]) -> None:
    ...
    lines = [
        "# Generated by launcher/setup_mse.py. Run that script again after moving MSE.",
...
class MSEConfig:
    ...
    def from_values(cls, values: Mapping[str, str]) -> "MSEConfig":
        ...
        if missing:
            keys = ", ".join(missing)
            raise RuntimeError(
                f"Missing MSE configuration ({keys}). Run `python launcher/setup_mse.py` first."
            )
