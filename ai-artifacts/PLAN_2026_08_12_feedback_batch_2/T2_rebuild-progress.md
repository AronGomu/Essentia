...
- [ ] 1. Write `tests/test_rebuild_progress.py`. Import both scripts with the `importlib.util.spec_from_file_location` pattern used by `tests/test_release_package.py`. For the phase test, build a temp open package by reusing the existing fixture helper in `tests/test_release_package.py` (copy the helper into the new file if it is not importable — duplication is fine, do not refactor the existing suite).
- [ ] 2. In `.script/export_mse_renders.py`, add next to `summary_line`:
...
- [ ] 3. Change `def export(project, output, config)` to `def export(project, output, config, *, quiet: bool = False)`. In its staging copy loop, replace `for key, card in expected_by_key.items():` with `for position, (key, card) in enumerate(expected_by_key.items(), start=1):` and add as first statement of the body `report_progress("mse.render", position, len(cards), card.name, quiet=quiet)`.
- [ ] 4. Same change in `export_print_masters`: signature gains `*, quiet: bool = False`, loop becomes `enumerate(expected_by_key.items(), start=1)`, first body statement `report_progress("mse.print", position, len(cards), card.name, quiet=quiet)`.
- [ ] 5. In `parse_args()`, add `parser.add_argument("--quiet", action="store_true", help="suppress per-card progress lines; keep the summary line")`.
- [ ] 6. In `main()`, pass `quiet=args.quiet` to every `export(...)` and `export_print_masters(...)` call (3 call sites: the `--attest-canonical` branch, the main `provenance = export(...)`, and the `--print-masters` branch).
- [ ] 7. In `.script/release_package.py`, add above `build_artifacts`:
...
- [ ] 8. Rewrite the body of `rebuild()` to wrap each step, keeping the existing order and return value:
...
- [ ] 9. Run `python .script/rebuild_open_packages.py` and eyeball: 5 phase pairs, 50 `mse.render` lines, 50 `mse.print` lines, then the two existing summary lines.
...
- [ ] `python -m unittest tests.test_rebuild_progress -v` — 4 tests pass
- [ ] `python -m unittest tests.test_release_package tests.test_export_mse_renders -v` — same failures as baseline (expected: none in these two files)
- [ ] `python .script/rebuild_open_packages.py 2>&1 | grep -c '^mse.render '` — prints `50`
- [ ] `python .script/rebuild_open_packages.py 2>&1 | grep -E '^rebuild .+ \[5/5\] validate done' | wc -l` — prints `1`
- [ ] `python -m unittest discover -s tests 2>&1 | tail -3` — failure count still 21
- [ ] manual check: `cd website && npm run cards:rebuild` shows live progress
- [ ] commit msg draft: `feat(rebuild): report phases and per-card render progress`
