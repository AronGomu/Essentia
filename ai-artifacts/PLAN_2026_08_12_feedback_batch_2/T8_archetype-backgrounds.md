...
- [x] 1. Write both test files. (`tests/unit/archetype-background.test.ts`, `tests/e2e/archetype-background.spec.ts`, both red before impl — `npx vitest run tests/unit/archetype-background.test.ts` → 5 failed)
- [x] 2. Create `website/scripts/make-archetype-backgrounds.mjs`, mirroring `make-hero-art.mjs`: (file created, mirrors `make-hero-art.mjs` structure)
...
- [x] 3. Add `"backgrounds:art": "node scripts/make-archetype-backgrounds.mjs"` to `website/package.json` scripts, next to `hero:art`. (`package.json` line added)
- [x] 4. Run `cd website && npm run backgrounds:art`; confirm both webp files exist and are under 600 KB (`ls -la public/backgrounds/`). Commit the two webp files (they are in `public/`, which is tracked — only `public/generated/` is ignored). (ran; `burning-abyss.webp` 188870 B, `nekroz.webp` 369978 B — both under 600 KiB)
- [x] 5. Edit `website/content/art-provenance.json`: keep `schemaVersion: 1`, replace `note` with text that states hero art is converted from Konami illustrations with sharp **and** that the two `/backgrounds/*.webp` page backdrops are AI-generated by the owner, and keep the two entries written by the script (sorted after the `/art/` entries is fine). (note edited; unit test `admits AI generation in the provenance note` passes)
...
- [x] 6. `BaseLayout.astro` props/style block done. (`npm run check` → 0 errors; CSP hashed style confirmed in built `dist/archetypes/nekroz/index.html`)
...
- [x] 7. `[slug].astro` updated. (unit test `is passed only by the archetype slug page` passes — only this file matches `background=`)
...
- [x] 8. `global.css` `body::before` rule updated. (unit test `layers atmosphere, dim veil, then the photo, in that order` passes)
...
- [x] `cd website && npx vitest run` — no new failures (766 passed, 1 pre-existing unrelated failure: `asset-rights.test.ts` — known-red from uncommitted MSE resave, not touched by this ticket)
- [x] `cd website && npm run check && npm run lint && npm run format:check` (all three: 0 errors)
- [x] `cd website && npm run build` — `harden-csp` must succeed, proving the injected `<style>` is hashed and no `unsafe-inline` survives (build succeeded, `csp: hashed inline content in 152 HTML files`)
- [x] `cd website && npm run budgets:check` (`budgets: 9 JS, 152 HTML, 217 images, 50 print masters (51 MiB) within limits`)
- [x] `cd website && npx playwright test tests/e2e/archetype-background.spec.ts tests/e2e/archetype-hero-panel.spec.ts` (18 passed across chromium/firefox/webkit)
